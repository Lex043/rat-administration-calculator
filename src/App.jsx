import { useState } from "react";

const App = () => {
    const [number, setNumber] = useState("");
    const [kilogram, setKilogram] = useState("");
    const [doseMg, setDoseMg] = useState("");
    const [doseMl, setDoseMl] = useState("");

    // Vitamin C
    const [vDoseMg, setVDoseMg] = useState("");
    const [vDoseMl, setVDoseMl] = useState("");

    const handleSubmit = (e) => {
        e.preventDefault();

        // Triclosan
        let kg = number / 1000;
        let dosepermg = Number(kg * 4.3).toFixed(4);
        let doseperml = Number(dosepermg / 2.5).toFixed(2);
        setKilogram(kg);
        setDoseMg(dosepermg);
        setDoseMl(doseperml);

        // Vitamin C
        let vdosepermg = Number(kg * 200).toFixed(1);
        let vdoseperml = Number(vdosepermg / 100).toFixed(2);
        setVDoseMg(vdosepermg);
        setVDoseMl(vdoseperml);
    };

    return (
        <main className="bg-[#150e28] min-h-screen text-white py-10 px-4 md:px-10">
            <section className="lg:w-[40%] lg:mx-auto">
                <header className="text-center font-sans">
                    <h1 className="font-bold">
                        DOSAGE CALCULATION FOR RAT ADMINISTRATION
                    </h1>
                    <p className="py-2 text-sm">For Triclosan and Vitamin C</p>
                    <p className="text-sm">Dr. Alamu&apos;s Project Group</p>
                </header>

                <form
                    onSubmit={handleSubmit}
                    className="flex flex-col gap-4 justify-center mt-6"
                >
                    <input
                        type="number"
                        required
                        value={number}
                        onChange={(e) => setNumber(e.target.value)}
                        placeholder="Enter your Rat weight in gram"
                        className="bg-transparent border-[1px] text-white outline-none rounded-md pl-2 py-2 w-full"
                    />

                    <button
                        className="w-[120px] rounded-md bg-gradient-to-r from-[#d434fe] via-[#d434fe] to-[#903aff]
                 py-3 font-montserrat text-sm text-white mx-auto"
                    >
                        Generate
                    </button>
                </form>

                <section className="mt-4 flex flex-col gap-6">
                    <div>
                        <h1 className="text-center uppercase font-semibold font-mono">
                            Triclosan
                        </h1>
                        <h1>Wt/kg = {kilogram}</h1>
                        <h1>Cal of Dose/mg = {doseMg}</h1>
                        <h1>Cal of Dose/ml = {doseMl}</h1>
                    </div>

                    <div>
                        <h1 className="text-center uppercase font-semibold font-mono">
                            Vitamin C
                        </h1>
                        <h1>Wt/kg = {kilogram}</h1>
                        <h1>Cal of Dose/mg = {vDoseMg}</h1>
                        <h1>Cal of Dose/ml = {vDoseMl}</h1>
                    </div>
                </section>
            </section>
        </main>
    );
};

export default App;
